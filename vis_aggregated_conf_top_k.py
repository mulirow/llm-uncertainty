...
def compute_rank_based_top_k_score(hint_answers):
    ...
    rank_matrix, num_options, top_k, id_to_element = convert_to_rank_matrix(
        hint_answers
    )
...
def search_vis_answers_top_k(result_dict, task_type):
    ...
    for key, value in result_dict.items():
        ...
        top1_answer_confs_alltrails = (
            {}
        )  # only consider the first answer from top-k answers
        topk_answer_confs_alltrails = {}
        topk_answer_ranks = {}
        # consider every trail during ensemble
        for trail, answers in hint_answers.items():
            top_k_conf = hint_confs[
                trail
            ]  # get the corresponding confidence list for this 'trail_i' or f'hint{i}'
        ...
        average_confs = {
            ans: sum(conf_lists) / len(conf_lists) / 100
            for ans, conf_lists in topk_answer_confs_alltrails.items()
        }
        ...
        average_confs = {
            ans: np.sum(conf_lists) / conf_sum
            for ans, conf_lists in top1_answer_confs_alltrails.items()
        }
        ...
        rank_based_topk_answer, rank_based_topk_score = compute_rank_based_top_k_score(
            hint_answers
        )
...
pdb.set_trace()
score_dict = search_vis_answers_top_k(
    data, args.task_type, num_ensemble=args.num_ensemble
)
