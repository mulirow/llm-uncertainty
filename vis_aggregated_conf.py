...
parser.add_argument(
    "--prompt_type",
    type=str,
    choices=["vanilla", "cot", "self_probing", "top_k", "multi_step"],
    default="vanilla",
)
...
parser.add_argument(
    ...
    type=int,
    default=1,
    help="number of queries to ensemble for a given question",
)
parser.add_argument(
...
result_file_error_log = osp.join(
    log_folder, input_file_name.replace(".json", "_visual_error.log")
)
...
if "hyperparameters" in data_results:
    ...
    assert args.use_cot == data_results["hyperparameters"]["use_cot"], (
        args.use_cot,
        data_results["hyperparameters"]["use_cot"],
    )
...
if args.dataset_name in ["BigBench_DateUnderstanding"]:
    ...
    raise NotImplementedError(
        f"Please specify the normal_option_list for this dataset {args.dataset_name}"
    )
...
def search_vis_answers(result_dict, task_type, prompt_type, sampling_type):
    ...
    score_dicts = {"real_answer": [], "scores": {}}
    ...
    for key, value in result_dict.items():
        ...
        assert len(hint_answers) == len(
            hint_confs
        ), "(len(hint_answers) should be equivalent to len(hint_confidences))"
        ...
        for trail, ans in hint_answers.items():
            ...
            conf = hint_confs[
                trail
            ]  # get the corresponding confidence list for this 'trail_i' or 'hint_i'
        ...
        consistency_answer, consistency_score = compute_consistency_score(
            hint_answers, sampling_type
        )
        ...
        def compute_avg_confidence(hint_confs, answer_confs_alltrails, sampling_type):
            ...
            average_confs = {
                ans: sum(conf_lists) / conf_sum
                for ans, conf_lists in answer_confs_alltrails.items()
            }
        ...
        avg_conf_option, avg_confidence = compute_avg_confidence(
            hint_confs, answer_confs_alltrails, sampling_type
        )

        if prompt_type == "multi_step":
            avg_step_conf_option, avg_step_confidence = compute_avg_confidence(
                hint_step_confs, answer_stepconfs_for_alltrails, sampling_type
            )
        ...
        if prompt_type == "multi_step":
            score_dicts["scores"]["avg_multistep_conf"]["answer"].append(
                avg_step_conf_option
            )
            score_dicts["scores"]["avg_multistep_conf"]["score"].append(
                avg_step_confidence
            )
...
score_dict = search_vis_answers(
    data, args.task_type, prompt_type=args.prompt_type, sampling_type=args.sampling_type
)
...
def plot_ece_diagram(y_true, y_confs, score_type):
    ...
    plt.savefig(
        osp.join(
            visual_folder, input_file_name.replace(".json", f"_ece_{score_type}.pdf")
        ),
        dpi=600,
    )


def plot_confidence_histogram_with_detailed_numbers(
    y_true, y_confs, score_type, acc, auroc, ece, use_annotation=True
):
    ...
    n_wrong, bins_wrong, patches_wrong = plt.hist(
        wrong_confs,
        bins=21,
        alpha=0.5,
        label="wrong answer",
    ...
    if args.use_cot:
        ...
        plt.title(
            f"COT: ACC {acc:.2f} / AUROC {auroc:.2f} / ECE {ece:.2f}",
            fontsize=16,
            fontweight="bold",
        )
    else:
        ...
        plt.title(
            f"ACC {acc:.2f} / AUROC {auroc:.2f} / ECE {ece:.2f}",
            fontsize=16,
            fontweight="bold",
        )
    ...
    plt.savefig(
        osp.join(
            visual_folder, input_file_name.replace(".json", f"_auroc_{score_type}.png")
        ),
        dpi=600,
    )
    plt.savefig(
        osp.join(
            visual_folder, input_file_name.replace(".json", f"_auroc_{score_type}.pdf")
        ),
        dpi=600,
    )
...
def plot_confidence_histogram(
    ...
    n_correct, bins_correct, patches_correct = plt.hist(
        ...
        align="mid",
        range=(-2.5, 102.5),
        bottom=np.histogram(wrong_confs, bins=21, range=(-2.5, 102.5))[0],
    )
...
for metric_name, values in score_dict["scores"].items():
    ...
    correct = [
        real_answers[i] == predicted_answers[i] for i in range(len(real_answers))
    ]
    ...
    plot_confidence_histogram(
        ...
        predicted_confs,
        metric_name,
        result_matrics[metric_name]["acc"],
        result_matrics[metric_name]["auroc"],
        result_matrics[metric_name]["ece"],
        use_annotation=True,
    )
...
if args.num_ensemble == 1:
    assert result_matrics["consistency"]["acc"] == result_matrics["avg_conf"]["acc"], (
        result_matrics["consistency"]["acc"],
        result_matrics["avg_conf"]["acc"],
    )
    assert (
        result_matrics["consistency"]["auroc"] == result_matrics["avg_conf"]["auroc"]
    ), (result_matrics["consistency"]["auroc"], result_matrics["avg_conf"]["auroc"])
    assert result_matrics["consistency"]["ece"] == result_matrics["avg_conf"]["ece"], (
        result_matrics["consistency"]["ece"],
        result_matrics["avg_conf"]["ece"],
    )

with open(output_file, "a") as f:
    f.write(
        "dataset_name,model_name,prompt_type,sampling_type,aggregation_type,acc,ece,auroc,auprc_p,auprc_n,use_cot,input_file_name,num_ensemble\n"
    )
    for key, values in result_matrics.items():
        f.write(
            f"{args.dataset_name},{args.model_name},{args.prompt_type},{args.sampling_type},{key},{values['acc']:.5f},{values['ece']:.5f},{values['auroc']:.5f},{values['aurc']:.5f},{values['auprc_p']:.5f},{values['auprc_n']:.5f},{args.use_cot},{input_file_name},{args.num_ensemble}\n"
        )
