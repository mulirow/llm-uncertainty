...
extract_options = {
    SCIENCEQA_DATASET: lambda hint_value, _: {
        chr(ord("A") + idx): option
        for idx, option in enumerate(hint_value["real_answer"]["options"])
    },
    GSM8K_DATASET: lambda *_: {},
    BIGBENCH_OBJECT_COUNTING: lambda *_: {},
    BIGBENCH_DATE_UNDERSTANDING: lambda _, question: {
        option_key: option_value
        for option_key, option_value in re.findall(r"\(([A-Z])\)\s*(.*?)\t", question)
    },
    SPORTUND: lambda _, question: {
        option_key: option_value
        for option_key, option_value in re.findall(r"\(([A-Z])\)\s*(.*?)\t", question)
    },
    STRATEGY_QA: lambda *_: {"A": "Yes", "B": "No"},
    BUSINESS_ETHICS: lambda hint_value, _: {
        chr(ord("A") + idx): option
        for idx, option in enumerate(hint_value["real_answer"]["options"])
    },
    PROFESSIONAL_LAW: lambda hint_value, _: {
        chr(ord("A") + idx): option
        for idx, option in enumerate(hint_value["real_answer"]["options"])
    },
...
for question, answer in result_data.items():
    ...
    for hint_key, hint_value in answer.items():
        ...
        try:
            if args.prompt_type in ["vanilla", "cot"]:
                extracted_answer, extracted_conf = extract_hint_response_vanilla(
                    hint_response, options, args.task_type, error_log_file
                )
            elif args.prompt_type == "self_probing":
                answer_placeholder, extracted_conf = (
                    extract_hint_response_self_evaluate(
                        hint_response, options, args.task_type, error_log_file
                    )
                )
                ...
                extracted_answer, extracted_conf = extract_hint_response_top_k(
                    hint_response, args.num_K, options, args.task_type, error_log_file
                )
            elif args.prompt_type == "multi_step":
                extracted_answer, extracted_conf, step_confs = (
        ...
        if args.sampling_type == "self_random":
            ...
            if hint_key in ["hint" + str(i) for i in range(1, 30)]:
                ...
                raise ValueError(
                    f"The hint key {hint_key} is not in the list: ['hint0', 'hint1', 'hint2', 'hint3', 'hint4', ....]"
                )
    ...
    if args.num_ensemble == 1:
        ...
        if (
            ...
            and predicted_answer_conf is not None
            and len(hint_answers) >= require_min_num_hints
        ):
            processed_data[question] = {
                "real_answer": real_answer,
                "predicted_answer": predicted_answer,
...
final_json = {
    ...
    "sample_tested": len(processed_data),
