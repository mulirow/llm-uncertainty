...
k = args.num_K
print(
    f"***Using top {k} results for {args.dataset_name} {args.task_type} {args.model_name}***"
)
...
sample_hint_prompt = generate_misleading_hint(
    hint_type="hint1" if args.num_ensemble > 1 else "hint0",
    task_type=args.task_type,
    question=sample_question,
    qa_dataset=qa_data,
)
sample_prompt = generate_prompt(
    prompt_description, question=sample_question, misleading_hint=sample_hint_prompt
)
print("\n-------\n", sample_prompt, "\n-------\n")
...
for idx, question in enumerate(qa_data.keys()):
    ...
    if args.sampling_type == "misleading":
        ...
        assert len(test_hints) <= len(
            hint_prompts
        ), f"number of hints {len(test_hints)} should be less than or equal to {len(hint_prompts)}; otherwise we need to add more hint prompts"
        for hint_type in test_hints:
            misleading_hint = generate_misleading_hint(
                hint_type, args.task_type, question, qa_data
            )
