...
def extract1(data):
    ...
    for i, step_match in enumerate(step_matches):
        ...
        result[step_key] = {
            ...
            "confidence": (
                int(confidence_matches[i]) if i < len(confidence_matches) else None
            ),
        }
...
def check_validity_answer_conf(
    extracted_answer, extracted_conf, task_type, error_log_file
):
    ...
    if task_type == "multi_choice_qa":
        ...
        if (extracted_answer not in "ABCDEFGHIJKLMNOPQRSTUVWXYZ") and (
            extracted_answer != "place_holder"
        ):
        ...
        raise ValueError(
            f"The task type {task_type} is not in the list: ['multi_choice_qa', 'open_number_qa']"
        )
...
def search_final_answer_for_multistep(
    text, options: Dict[str, str], task_type, error_log_file
):
    ...
    def postprocess_match_without_option(match) -> Tuple[str, str]:
        ...
        for option_key, option_value in options.items():
            ...
            if (
                answer_option_value in option_value
                or option_value in answer_option_value
            ):
    ...
    patterns_and_postprocess_multi_choice.extend(
        [(pat, default_postprocess_match) for pat in patterns_multi_choice]
    )
    patterns_and_postprocess_multi_choice.extend(
        [
            (pat, postprocess_match_without_option)
            for pat in patterns_multi_choice_without_option
        ]
    )
    ...
    patterns_and_postprocess_open_number = [
        (pat, default_postprocess_match) for pat in patterns_open_number
    ]
    # pre-process
    ...
    for pattern, match_processor in patterns_and_postprocess:
        ...
        answer, conf = check_validity_answer_conf(
            answer, conf, task_type, error_log_file
        )
    ...
    if not is_match:
        ...
        with open(error_log_file, "a") as f:
            f.write(
                f"No `final answer and confidence` match found. extracted_answer: {answer}, extracted_conf: {conf}\n"
            )
        answer = None
        conf = None
...
def search_step_confidence(text, error_log_file):
    ...
    if is_match:
        ...
        with open(error_log_file, "a") as f:
            ...
            f.write(
                f"No `step confidence` match found for {text}, set confidence to 100\n"
            )
            f.write("-" * 60 + "\n")
            conf = 100
...
def extract_hint_response_multistep(
    data, options: Dict[str, str], task_type, error_log_file
):
    ...
    # Extract final answer and final confidence
    final_answer, final_confidence = search_final_answer_for_multistep(
        data, options, task_type, error_log_file
    )
    ...
    for idx, (step_no, step_sentence) in enumerate(step_matches):
        if int(step_no) != idx + 1:
            with open(error_log_file, "a") as f:
                f.write(
                    f"STEP NUMBER MISMATCH: step_no {step_no} != idx {idx} in the sentence ({step_sentence})\n"
                )
...
def extract_hint_response_vanilla(
    text, options: Dict[str, str], task_type, error_log_file
):
    def default_postprocess_match(match) -> Tuple[str, str]:
        assert match is not None
    ...
    patterns_and_postprocess_multi_choice.extend(
        [(pat, postprocess_match_without_option) for pat in patterns_multi_choice_werid]
    )
    ...
    patterns_and_postprocess_open_number = [
        (pat, postprocess_match_open_number) for pat in patterns_open_number
    ]
    # pre-process
    text = text.replace("(1-100)", "(0-100)")
...
def extract_hint_response_top_k(
    text, K, options: Dict[str, str], task_type, error_log_file
):
    def default_postprocess_match(match) -> Tuple[str, str]:
        assert match is not None
        option_key, conf_scale = match.group(1), match.group(2)
...
def extract_hint_response_self_evaluate(
    text, options: Dict[str, str], task_type, error_log_file
):
    def default_postprocess_match(match) -> Tuple[str, str]:
        assert match is not None
    ...
    # sometimes the LLM will directly output the answer rather than the associated option key
    patterns_multi_choice_without_option = [r"Confidence:\s*(.*?)\s*,\s*(\d+)%*"]
...
def extract_hint_response_confidence_scale(
    text, options: Dict[str, str], task_type, error_log_file
):
    ...
    for step_match in step_matches:
        ...
        for confidence_match in confidence_matches:
            if step_analysis.find(confidence_match) != -1:
                ...
                break
        result[step_key] = {"analysis": step_analysis, "confidence": step_confidence}
    result["final_answer"] = final_answer
    result["final_confidence"] = final_confidence
    return result
